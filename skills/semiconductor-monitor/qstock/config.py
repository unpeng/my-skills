...
# ============================================================
# ATR (Average True Range) 波动率自适应风控参数
# 借鉴 abu（阿布量化）：用真实波幅衡量波动，止损/做T价差随波动自适应，
# 避免固定百分比在高波动日过窄、低波动日过宽。以下均为可配置默认值。
# ============================================================
ATR_PERIOD = 14              # ATR 计算周期（标准14日，用Wilder平滑）
ATR_STOP_MULTIPLIER = 2.5    # ATR止损倍数 n：止损位 = 成本 - n×ATR
ATR_T_MULTIPLIER = 1.0       # 做T价差倍数 k：做T买入/卖出位 = 现价 ∓ k×ATR
ATR_TRAIL_MULTIPLIER = 2.0   # 移动止盈倍数 m：移动止盈位 = 区间最高 - m×ATR
ATR_TRAIL_WINDOW = 20        # 移动止盈参考的区间最高价窗口（交易日）
